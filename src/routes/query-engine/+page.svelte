<script lang="ts">
  ...
  // pendingHistoryId is $state so the finalize $effect re-runs if the
  // id is set after the status flip (e.g. if onStart ever races behind
  // beforeQuery). The other locals don't carry that ordering risk.
  let pendingHistoryId = $state<string | null>(null);
  let runStartedAt = 0;
  let queryRows = 0;

<script lang="ts">
  ...
  // The DbState (and its DuckDB worker) lives in +layout.svelte and
  // survives navigation between routes; SchemaState + this component
  // do not. Without this teardown, navigating away from /query-engine
  // would leak the registered file buffers and orphan local.main.*
  // views that the next visit's fresh SchemaState wouldn't know about.
  onDestroy(() => {
  ...
  const handleFiles = async (list: FileList | File[] | null | undefined) => {
    if (!list) return;
    // Snapshot the input's FileList synchronously. The caller resets
    // `input.value` after handing us `target.files` so a new selection
    // can fire change again — that reset empties the live FileList,
    // so we must materialize a stable array before any await.

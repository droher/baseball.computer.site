...
The production query engine uses DuckLake 1.0 through DuckDB-WASM. `src/lib/io/data-source.ts` defines the versioned catalog and schema URLs. The `bc_remote` alias remains for compatibility with shared SQL. Browser initialization loads the DuckLake extension and attaches read-only; local uploads use a separate writable in-memory database. Schema metadata comes from the same DuckLake publication as the data.
...
Zero server-side query execution. All SQL runs in the browser via DuckDB-WASM against a public DuckLake 1.0 catalog and Parquet files hosted on Cloudflare R2.
...
- On `init()`: `INSTALL ducklake; LOAD ducklake; ATTACH 'ducklake:https://data.baseball.computer/baseball/v1/baseball.ducklake' AS bc_remote (READ_ONLY); ATTACH ':memory:' AS local; USE bc_remote; SET SCHEMA=main_models`. The DuckLake catalog references immutable Parquet files; DuckDB-WASM streams what it needs over HTTP range requests. The `local` in-memory database is the writable target for user-uploaded files (read-only `bc_remote` rejects `CREATE VIEW`).
...
`+page.ts` (prerendered) fetches `https://data.baseball.computer/baseball/v1/catalog.json` at build time and flattens it into a `{ "schema.table": [columns] }` map. The map seeds a `SchemaState` instance in the page; `QueryInput` reads `schemaState.tables` for autocomplete and the sidebar tree reads `schemaState.bySchema`.
...
- `LocalFiles` accepts CSV / Parquet via either `<input type="file">` or a drag-and-drop zone (hidden under `md:`). Files persist only in worker memory — cleared on reload, and on `onDestroy` the component unregisters its files so navigating away from `/query-engine` doesn't leak buffers or orphan `local.main.*` views. Toast surfaces this on first registration.
...
1. **Public storage** — DuckLake catalog and Parquet responses must allow cross-origin GET/HEAD and HTTP Range requests. Verify on the production domain after publishing. Data files are immutable. The DuckLake catalog is read at browser initialization, while schema metadata is prerendered and requires a site rebuild after schema changes.

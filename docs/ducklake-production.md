...
DuckDB-WASM is pinned to a build that supports DuckLake 1.0. Test upgrades with a real DuckLake query; successful `SELECT 1` alone does not prove data access. The browser tests verify the attachment type, remote Parquet reads, downloads, schema completion, history, and local uploads. `PLAYWRIGHT_BASE_URL=https://baseball.computer pnpm test:integration` tests the deployed site without starting a local server.
